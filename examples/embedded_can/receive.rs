//! embedded-can receive example
//!
//! The node participates fully in the CAN protocol (sends ACK bits) but
//! does not transmit application frames. Received frames are polled from
//! the internal queue via `embedded_can::nb::Can::receive()` and logged
//! via defmt; delta statistics are printed every second.
//!
...
use defmt_rtt as _;
use embedded_can::{nb::Can as NbCan, Frame, Id};
...
fn main() -> ! {
    ...
    loop {
        ...
        for _ in 0..100 {
            ...
            cortex_m::asm::delay(1_250_000); // ~10ms at 125MHz
        }
