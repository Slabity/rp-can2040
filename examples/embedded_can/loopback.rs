...
//! CAN bus. CAN0 transmits a frame every 500ms; CAN1 polls its receive
//! queue via `embedded_can::nb::Can::receive()` and sends a reply. Delta
//! statistics and bus-off recovery are included.
//!
//! Wiring:
...
use defmt_rtt as _;
use embedded_can::{nb::Can as NbCan, Frame, Id, StandardId};
...
fn on_can0_event(notification: Notification) {
    if let Notification::Error = notification {
        error!("CAN0 error");
    }
}
...
fn main() -> ! {
    ...
    loop {
        ...
        let stats0 = cortex_m::interrupt::free(|cs| {
            if let Some(can) = CAN0.borrow(cs).borrow_mut().as_mut() {
                match NbCan::transmit(can, &frame) {
                    Ok(_displaced) => info!("CAN0 TX: {:?}", frame),
                    Err(nb::Error::WouldBlock) => error!("CAN0 TX queue full"),
                    Err(nb::Error::Other(_)) => error!("CAN0 TX error"),
                }
                Some(can.statistics())
            } else {
                None
        ...
        // CAN0: drain the receive queue (replies from CAN1 arrive here).
        loop {
            let maybe = cortex_m::interrupt::free(|cs| {
                CAN0.borrow(cs).borrow_mut().as_mut().and_then(|can| NbCan::receive(can).ok())
            });
            match maybe {
                Some(rx) => match rx.id() {
        ...
        // CAN1: drain the receive queue and reply to each frame.
            ...
            if let Some(can) = CAN1.borrow(cs).borrow_mut().as_mut() {
                while let Ok(rx) = NbCan::receive(can) {
                    ...
                    match NbCan::transmit(can, &reply) {
                        ...
                        Err(nb::Error::Other(_)) => error!("CAN1 TX error"),
                    }
                }
                Some(can.statistics())
            } else {
